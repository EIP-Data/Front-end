...
<template>
  <LayoutDashboard>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ...
      >
        <div aria-hidden="true" class="w-10 h-10 rounded-lg bg-amber-100 dark:bg-amber-900/30 flex items-center justify-center flex-shrink-0">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex items-center gap-4">
        <div aria-hidden="true" class="w-10 h-10 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center flex-shrink-0">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex items-center gap-4">
        <div aria-hidden="true" class="w-10 h-10 rounded-lg bg-purple-100 dark:bg-purple-900/30 flex items-center justify-center flex-shrink-0">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex items-center gap-4">
        <div aria-hidden="true" class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-gray-700 flex items-center justify-center flex-shrink-0">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
      <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
        <h2 id="users-table-heading" class="text-base font-semibold text-gray-800 dark:text-white">{{ t('adminPanelView.users.title') }}</h2>
        <button
          type="button"
          @click="fetchUsers"
          ...
          :aria-label="t('adminPanelView.users.refresh')"
        >
      ...
      <div v-if="isLoading" role="status" aria-live="polite" class="flex items-center justify-center py-16">
        <svg aria-hidden="true" class="animate-spin w-6 h-6 text-brand" fill="none" viewBox="0 0 24 24">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/>
        </svg>
      </div>

      <div v-else-if="loadError" role="alert" class="flex flex-col items-center justify-center py-16 gap-3">
        <p class="text-sm text-red-500">{{ loadError }}</p>
        <button type="button" @click="fetchUsers" class="text-sm text-brand hover:underline">{{ t('adminPanelView.users.retry') }}</button>
      </div>

      <div v-else-if="users.length > 0" class="overflow-x-auto">
        <table class="w-full text-sm" aria-labelledby="users-table-heading">
          <caption class="sr-only">{{ t('adminPanelView.users.title') }}</caption>
          <thead>
            <tr class="bg-gray-50 dark:bg-gray-700/50 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
              <th scope="col" class="px-6 py-3">{{ t('adminPanelView.users.columns.user') }}</th>
              <th scope="col" class="px-6 py-3">{{ t('adminPanelView.users.columns.email') }}</th>
              <th scope="col" class="px-6 py-3">{{ t('adminPanelView.users.columns.role') }}</th>
              <th scope="col" class="px-6 py-3 text-right">{{ t('adminPanelView.users.columns.actions') }}</th>
            </tr>
          </thead>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  <div aria-hidden="true" class="w-8 h-8 rounded-full bg-brand/10 dark:bg-brand/20 flex items-center justify-center text-brand font-semibold text-sm flex-shrink-0">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center justify-end gap-2">
                  <button
                    ...
                    :aria-label="t('adminPanelView.users.promote') + ' ' + user.username"
                  >
                  ...
                  <button
                    ...
                    :aria-label="t('adminPanelView.users.demote') + ' ' + user.username"
                  >
                  ...
                  <button
                    ...
                    :aria-label="t('adminPanelView.users.delete') + ' ' + user.username"
                  >
        ...
        >
          ...
          >
            <div class="flex items-center gap-3 mb-4">
              <div aria-hidden="true" class="w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-red-600 dark:text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
              </div>
              <h3 id="delete-user-modal-title" class="text-lg font-semibold text-gray-900 dark:text-white">{{ t('adminPanelView.deleteModal.title') }}</h3>
            </div>
